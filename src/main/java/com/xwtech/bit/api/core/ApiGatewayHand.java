package com.xwtech.bit.api.core;
...
import com.xwtech.bit.api.common.Md5Util;
...
import com.xwtech.bit.api.common.UtilJson;
import com.xwtech.bit.api.core.ApiStore.ApiRunnable;
import com.xwtech.bit.api.exception.ApiException;
import com.xwtech.bit.api.pojo.Token;
import com.xwtech.bit.api.pojo.params;
...
public class ApiGatewayHand implements InitializingBean, ApplicationContextAware {
    ...
    private ApiRequest buildApiRequest(HttpServletRequest request) {
        ...
        apiRequest.setAccessToken(request.getParameter(params.TOKEN));
        apiRequest.setSign(request.getParameter(params.SIGN));
        apiRequest.setTimestamp(request.getParameter(params.TIMESTAMP));
        apiRequest.seteCode(request.getParameter(params.ECODE));
        apiRequest.setuCode(request.getParameter(params.UCODE));
        apiRequest.setParams(request.getParameter(params.PARAMS));
        return apiRequest;
    }

    private ApiRequest signCheck(ApiRequest request) throws ApiException {
        Token token = tokenService.getToken(request.getAccessToken()).get(0);
        if (token == null ) {
        ...
        String sign = Md5Util.MD5(secret + methodName + params + accessToken + timestamp + secret);

        if (!sign.toUpperCase().equals(request.getSign())) {//转换为大写区比较
...
//        result.put("stack", out.toString());
    ...
    private ApiRunnable sysParamsValdate(HttpServletRequest request) throws ApiException {
        ...
        String param = request.getParameter(params.PARAMS);
        String token = request.getParameter(params.TOKEN);
        String sign = request.getParameter(params.SIGN);
        String timestamp = request.getParameter(params.TIMESTAMP);
        String eCode = request.getParameter(params.ECODE);
        String uCode = request.getParameter(params.UCODE);
        ApiRunnable api;
        if (method == null || method.trim().equals("")) {
            ...
            throw new ApiException("调用失败：参数'sign'为空");
        }else if (timestamp == null) {
            throw new ApiException("调用失败：参数'timestamp'为空");
        }else if (eCode == null) {
            throw new ApiException("调用失败：参数'eCode'为空");
        }else if (uCode == null) {
            throw new ApiException("调用失败：参数'uCode'为空");
        }else if (token ==null) {
            throw new ApiException("调用失败：参数'token'为空");
        }else if ((api = apiSorte.findApiRunnable(method)) == null) {
